...
export abstract class Type<V> {
  ...
  protected constructor(schema: AnySchema | any, options: TypeOptions<V> = {}) {
  ...
  public validate(value: any, context: Record<string, any> = {}, namespace?: string): V | any {
  ...
  private onError(error: SchemaTypeError | ValidationErrorItem): SchemaTypeError {
    ...
    const { context = {}, type, path, message }: any = error;
